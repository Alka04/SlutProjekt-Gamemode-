        ...
        for tile in world.obstacle_list:
            ...
            #Kolla för kollision i y riktningen
                if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                    ...
                    #Kolla om man är över marken, i.e. Faller
                    elif self.vel_y >= 0:
                        self.vel_y = 0
                        self.in_air = False
        ...
        for y, row in enumerate(data):
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    elif tile >= 9 and tile <= 10:#skapa vatten
                        water = Water(img, x * TILE_SIZE, y * TILE_SIZE)
                        water_group.add(water)
                    elif tile >= 11 and tile <= 14: #skapa dekorationer
                        decoration = Decoration(img, x * TILE_SIZE, y * TILE_SIZE)
                        decoration_group.add(decoration)
                    elif tile == 15:#skapa spelare
                        player = Soldier('player', x * TILE_SIZE, y * TILE_SIZE, 1.65, 5, 20, 5)
                        health_bar = HealthBar(10, 10, player.health, player.health)
                    elif tile == 16:#skapa fiender
                        enemy = Soldier('enemy', x * TILE_SIZE, y * TILE_SIZE, 1.65, 2, 20, 0)
                        enemy_group.add(enemy)
                    elif tile == 17:#skapa ammo lådor
                        item_box = ItemBox('Ammo', x * TILE_SIZE, y * TILE_SIZE)
                        item_box_group.add(item_box)
                    elif tile == 18:#skapa granat lådor
...
class Grenade(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #Kontorllera kollision med nivån
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                self.speed = 0
                #Kolla om man är under marken, i.e. kasta uppåt
                if self.vel_y < 0:
                    self.vel_y = 0
                    dy = tile[1].bottom - self.rect.top
                #Kolla om man är över marken, i.e. Faller
